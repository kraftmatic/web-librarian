...
function WEBLIB_dump_tables()
{
  ...
  foreach ($tables as $table_a) {
    foreach ($table_a as $table) {
      ?><h3><?php _e('Table: ','web-librarian'); echo $table; ?></h3>
...
class WEBLIB_Patron {
	...
	function expiration() { return $this->record['expiration']; }
	...
	static function PatronIdDropdown($selected,$args) {
	  ...
	  extract (wp_parse_args($args, array('onlyunassoc' => false,
					      ...
					      'selectclass' => '',
					      'labelclass' => '')));
	  echo '<!-- *** WEBLIB_Patron::PatronIdDropdown: onlyunassoc = '.$onlyunassoc." -->\n";
	  echo '<!-- *** WEBLIB_Patron::PatronIdDropdown: selected = '.$selected." -->\n";
	  ?><label for="<?php echo $name; ?>"<?php
		if ($labelclass != '') { echo ' class="'.$labelclass.'"'; }
	  ?>><?php echo $label; ?></label><?php echo $beforesel; 
	  ?><select id="<?php echo $name; ?>" name="<?php echo $name; ?>"<?php
		echo ' class="patrondroplist';
		if ($selectclass != '') { echo ' '.$selectclass; }
		echo '"';
	  ?>>
	  <option value="0"<?php if ($selected == 0) { echo ' selected="selected"'; } ?>></option>
	  <?php
		$patrons = WEBLIB_Patron::AllPatrons();
		foreach ($patrons as $patron) {
		  if ($onlyunassoc) {
		    $userid = WEBLIB_Patron::UserIDFromPatronID($patron['patronid']);
		    echo '<!-- *** WEBLIB_Patron::PatronIdDropdown: userid = '.$userid." -->\n";
		    if ($userid != -1) {continue;}
		  }
		  ?><option value="<?php echo $patron['patronid']; ?>"<?php
			if ( $patron['patronid'] == $selected ) {
			  echo ' selected="selected"';
			} ?>><?php echo $patron['name']; ?></option>
		  <?php
		}
		?></select><?php echo $aftersel; ?>
	   <?php
	}
...
class WEBLIB_ItemInCollection {
	...
	static function AllBarCodes($orderby = 'barcode', $order = 'ASC') {
	  global $wpdb;
	  $olderror = $wpdb->show_errors(get_option('weblib_debugdb') != 'off');
	  $rawbarcodes = $wpdb->get_col('SELECT barcode FROM '.
			WEBLIB_COLLECTION.' order by '.$orderby.' '.$order);
	  $wpdb->show_errors($olderror);
	  $barcodes = array();
